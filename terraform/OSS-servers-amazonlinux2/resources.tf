...
terraform {
  backend "s3" {
    bucket = "performance-cto-group"
    key    = "benchmarks/infrastructure/perf-cto-OSS-servers-amazonlinux2.tfstate"
    region = "us-east-1"
  }
}
...
resource "aws_instance" "perf_cto_server_c5n_9xlarge" {
  ...
  ###############
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/${var.os}/redis-cluster-oss.yml -i ${self.public_ip}, --extra-vars \"redis_version=${var.redis_oss_version} redis_cluster_shard_count=${var.redis_shards_per_vm}\""
  }
}
