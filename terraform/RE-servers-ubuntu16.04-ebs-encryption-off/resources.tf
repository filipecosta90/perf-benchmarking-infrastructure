...
terraform {
  backend "s3" {
    bucket = "performance-cto-group"
    key    = "benchmarks/infrastructure/perf-cto-RE-servers-ubuntu16.04-ebs-encryption-off.tfstate"
    region = "us-east-1"
  }
}
...
resource "aws_instance" "perf_cto_server" {
  ...
  ##############
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/${var.os}/1VM-redis-enterprise-ebs-encryption-off.yml -i ${self.public_ip}.inv"
  }
