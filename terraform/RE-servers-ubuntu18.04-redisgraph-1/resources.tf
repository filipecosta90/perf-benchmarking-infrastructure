...
terraform {
  backend "s3" {
    bucket = "performance-cto-group"
    key    = "benchmarks/infrastructure/perf-cto-RE-servers-ubuntu18.04-redisgraph-1.tfstate"
    region = "us-east-1"
  }
}
...
resource "aws_instance" "perf_cto_server" {
  ...
  ##########################
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/${var.os}/redisgraph-oss.yml -i ${self.public_ip},"
  }
  ...
  ##############
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/common/1VM-redis-enterprise-redisgraph.yml --extra-vars \"re_cluster_name=${var.re_cluster_name} re_proxy_max_threads=10 db_shards_count=1 re_proxy_threads=10\" -i ${self.public_ip}.inv"
  }
