...
resource "aws_instance" "az3_master_instance" {
  ...
  volume_tags = {
    Name        = "ebs_block_device-${var.setup_name}-${data.aws_availability_zones.available.names[2]}-master"
    RedisModule = "${var.redis_module}"
  }

  tags = {
    Name        = "${var.setup_name}-${data.aws_availability_zones.available.names[2]}-master"
    RedisModule = "${var.redis_module}"
  }
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      l2 = "${self.public_ip}"
      l3 = "[re_slave]"
      l4 = "${aws_instance.az3_replica_instance.public_ip}"
    }
  }
