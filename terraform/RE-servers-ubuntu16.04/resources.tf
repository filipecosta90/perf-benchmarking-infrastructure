...
terraform {
  backend "s3" {
    bucket = "performance-cto-group"
    key    = "benchmarks/infrastructure/perf-cto-RE-servers-ubuntu16.04.tfstate"
    region = "us-east-1"
  }
}
...
resource "aws_instance" "perf_cto_server_c5n_9xlarge" {
  ...
  ###############
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/${var.os}/fromsource.yml -i ${self.public_ip},"
  }
