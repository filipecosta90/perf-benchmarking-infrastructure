...
resource "aws_instance" "az1_replica_instance" {
  ...
  provisioner "local-exec" {
    ...
    command = "echo $l3 >> ${self.public_ip}.inv && echo $l4 >> ${self.public_ip}.inv"
  ...
  ##############
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/common/2VM-1-master-1-replica-multi-az-az1-replica.yml --extra-vars \"re_cluster_name=${var.re_cluster_name}-${data.aws_availability_zones.available.names[0]} re_proxy_max_threads=10 db_shards_count=16 re_proxy_threads=10\" -i ${self.public_ip}.inv"
  }
