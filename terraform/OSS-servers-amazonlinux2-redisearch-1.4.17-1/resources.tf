...
terraform {
  backend "s3" {
    bucket = "performance-cto-group"
    key    = "benchmarks/infrastructure/perf-cto-OSS-servers-amazonlinux2-redisearch-1.4.17-1.tfstate"
    region = "us-east-1"
  }
}
...
resource "aws_instance" "perf_cto_server" {
  ...
  ##########################
  # Install RediSearch OSS #
  ##########################
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/${var.os}/redisearch-oss.yml -i ${self.public_ip}, --extra-vars \"redisearch_version=${var.redisearch_version}\""
  }
