...
terraform {
  backend "s3" {
    bucket = "performance-cto-group"
    key    = "benchmarks/infrastructure/perf-cto-RE-servers-ubuntu18.04-redisearch-15.tfstate"
    region = "us-east-1"
  }
}
...
resource "aws_instance" "perf_cto_server" {
  ...
  ##############
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ${var.ssh_user} --private-key ${var.private_key} ../../playbooks/common/1VM-redis-enterprise-redisearch.yml --extra-vars \"re_cluster_name=${var.re_cluster_name} re_proxy_max_threads=10 db_shards_count=15 re_proxy_threads=10\" -i ${self.public_ip}.inv"
  }
